...
func validateGameVariable(entry GameVariable, tagVocab map[string]struct{}, variableSlugs map[string]struct{}, mechanicSlugs map[string]struct{}) error {
	...
	if entry.SchemaVersion != VariableSchemaVersion && entry.SchemaVersion != "1.1" {
		return fmt.Errorf("%s: schema_version must be %q or %q", label, VariableSchemaVersion, "1.1")
	}
...
func validateUIMenu(entry UIMenu, tagVocab map[string]struct{}, menuSlugs map[string]struct{}, variableSlugs map[string]struct{}, mechanicSlugs map[string]struct{}) error {
	...
	if entry.SchemaVersion != UIMenuSchemaVersion && entry.SchemaVersion != "1.1" {
		return fmt.Errorf("%s: schema_version must be %q or %q", label, UIMenuSchemaVersion, "1.1")
	}
	if err := validateSlug(label, entry.Slug); err != nil {
		return err
	}
...
func validateSkillCategory(label string, c SkillCategory) error {
	switch c {
	case SkillMotor, SkillCognitive, SkillStrategic, SkillSocial, SkillCreative:
		return nil
	default:
		return fmt.Errorf("%s: invalid category %q", label, c)
	}
...
func validateDesignSkill(entry DesignSkill, tagVocab map[string]struct{}, mechanicSlugs map[string]struct{}, variableSlugs map[string]struct{}) error {
	label := entry.Slug
	if label == "" {
		label = entry.Name
	}
	if entry.SchemaVersion != SkillSchemaVersion {
		return fmt.Errorf("%s: schema_version must be %q", label, SkillSchemaVersion)
	}
	if err := validateSlug(label, entry.Slug); err != nil {
		return err
	}
	...
	if entry.LearningOutcome == "" {
		return fmt.Errorf("%s: learning_outcome is required", label)
	}
	if err := validateSkillCategory(label, entry.Category); err != nil {
		return err
	}
	if len(entry.Tags) == 0 {
		return fmt.Errorf("%s: at least one tag is required", label)
...
func validateSkillsCatalog(root string, catalog SkillsCatalog, mechanicSlugs map[string]struct{}, variableSlugs map[string]struct{}) error {
	if catalog.SchemaVersion != "" && catalog.SchemaVersion != SkillSchemaVersion {
		return fmt.Errorf("skills catalog schema_version must be %q", SkillSchemaVersion)
	}
	tagVocab, err := LoadSkillTags(root)
	if err != nil {
		return err
	}
	for _, entry := range catalog.Skills {
		if err := validateDesignSkill(entry, tagVocab, mechanicSlugs, variableSlugs); err != nil {
			return err
		}
	}
	return nil
