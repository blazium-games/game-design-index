...
export async function registerQueryTools(deps: WebMCPDeps, opts: RegisterOpts) {
  ...
  await mc.registerTool(
    {
      name: 'get-catalog',
      description: 'Return corpus statistics, schema version, and genre list for the Game Design Index.',
  ...
  await mc.registerTool(
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          type: { type: 'string', enum: ['game', 'mechanic', 'variable', 'menu', 'skill'], description: 'Optional type filter' },
  ...
  await mc.registerTool(
    {
      name: 'list-skills',
      description: 'List design skill catalog entries.',
      inputSchema: {
        type: 'object',
        properties: {
          category: { type: 'string' },
      ...
      async execute(args) {
        const { category, limit = 50 } = args as { category?: string; limit?: number }
        let rows = await deps.api.fetchSkillsIndex()
  ...
  await mc.registerTool(
    {
      name: 'get-skill',
      description: 'Return a full design skill catalog entry by slug.',
      inputSchema: {
        type: 'object',
        properties: { slug: { type: 'string' } },
        required: ['slug'],
      ...
      async execute(args) {
        const { slug } = args as { slug: string }
        return jsonResponse(await deps.api.fetchSkill(slug))
      },
    },
    opts,
  )
  ...
  await mc.registerTool(
    {
      name: 'get-analytics',
      description: 'Return pre-computed corpus analytics: stats, correlations, heatmaps, and insights.',
      inputSchema: { type: 'object', properties: {} },
      async execute() {
        return jsonResponse(await deps.api.fetchAnalytics())
      },
    },
    opts,
  )
